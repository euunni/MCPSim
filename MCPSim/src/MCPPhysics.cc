...
Matrix3x3 Physics::Transporter1(const Matrix3x3& Mat, double t, double cts, double alpha) {
    double x = Mat(0, 0) + Mat(1, 0) * t + cts * (t * t) / 2;
    double y2 = Mat(0, 1) + Mat(1, 1) * t;
    double z2 = Mat(0, 2) + Mat(1, 2) * t;
...
bool Physics::Erreur(const Matrix3x3& Mat, int n, double alpha, double x0, double pas, double dia, double R) {
    double y_r = (Mat(0, 1) - tan(alpha) * Mat(0, 0) + tan(alpha) * x0 - (pas + dia) * n - (pas + dia) / 2);
    double z_r = Mat(0, 2);
...
Matrix3x3 Physics::TransportGap(const Matrix3x3& Mat, double t, double cts) {
    double x = Mat(0, 0) + Mat(1, 0) * t + cts * (t * t) / 2;
    double y2 = Mat(0, 1) + Mat(1, 1) * t;
    double z2 = Mat(0, 2) + Mat(1, 2) * t;
    double vx = Mat(1, 0) + cts * t;
